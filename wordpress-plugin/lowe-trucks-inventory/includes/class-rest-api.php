...
class LTI_REST_API {
	...
	private static function format_truck( WP_Post $post, string $lang ): array {
		$localized = LTI_Meta_Fields::get_localized_block( $post->ID, $lang );
		...
		return array(
			...
			'i18n'        => self::build_i18n_block( $post->ID ),
		);
	}
	...
	private static function build_i18n_block( int $post_id ): array {
		$block = array();
		foreach ( LTI_Meta_Fields::LANGS as $lang ) {
			$block[ $lang ] = LTI_Meta_Fields::get_localized_block( $post_id, $lang );
		}
		return $block;
	}
}
