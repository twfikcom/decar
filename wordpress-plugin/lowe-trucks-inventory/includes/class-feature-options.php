...
class LTI_Feature_Options {
	...
	/**
	 * Passenger-vehicle feature groups (reference: dealership inventory UI).
	 *
	 * @return array<string, array{label: array<string, string>, items: array<string, array<string, string>>}>
	 */
	private static function passenger_groups(): array {
		return array(
			'comfort' => array(
				'label' => array(
					'de' => 'Komfort',
			...
			'other' => array(
				...
				'items' => array(
					...
					'wheels_17' => array( 'de' => '17-Zoll-Felgen', 'en' => '17\'\' Wheels', 'ar' => 'جنوط 17 بوصة' ),
				),
			),
		);
	}
	...
	/**
	 * Feature groups for trucks: passenger options plus commercial-specific groups.
	 *
	 * @return array<string, array{label: array<string, string>, items: array<string, array<string, string>>}>
	 */
	public static function truck_groups(): array {
		$passenger = self::passenger_groups();

		$passenger['safety']['items'] = array_merge(
		...
		$passenger['other']['items'] = array_merge(
			$passenger['other']['items'],
			array(
